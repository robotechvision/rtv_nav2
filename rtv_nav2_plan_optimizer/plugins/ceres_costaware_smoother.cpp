...
#include "rtv_nav2_plan_optimizer/plugins/ceres_costaware_smoother.hpp"
...
void CeresCostawareSmoother::cleanup()
{
  RCLCPP_INFO(
    ...
    " rtv_nav2_plan_optimizer::CeresCostawareSmoother",
    plugin_name_.c_str());
}
...
nav_msgs::msg::Path CeresCostawareSmoother::optimizePath(const nav_msgs::msg::Path & path)
{
  ...
  for (int i = 0; i < (int)path.poses.size()-1; i++) {
    ...
    // RCLCPP_INFO(rclcpp::get_logger("optimizer_server"), "pos: %lf %lf, or: %lf %lf, mvmt: %lf %lf, rev: %lf",
    //             path_world.back()[0], path_world.back()[1], orientation[0], orientation[1], mvmt[0], mvmt[1], path_world.back()[2]);
  }
  path_world.emplace_back(path.poses.back().pose.position.x, path.poses.back().pose.position.y, path_world.back()[2]);
  // RCLCPP_INFO(rclcpp::get_logger("optimizer_server"), "pos: x: %lf, y: %lf, rev: %lf", path_world.back()[0], path_world.back()[1], path_world.back()[2]);
...
PLUGINLIB_EXPORT_CLASS(
  rtv_nav2_plan_optimizer::CeresCostawareSmoother,
  rtv_nav2_plan_optimizer::PlanOptimizer)
